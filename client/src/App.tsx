...
// Tipagem do Usuário
interface User {
  id: string; // <--- O ID É CRUCIAL AGORA
  username: string;
  avatarUrl: string;
  name: string;
  pet?: {
...
function App() {
  ...
  // --- NOVOS ESTADOS PARA BATALHA ---
  const [rival, setRival] = useState<User | null>(null);
  ...
  const handleLogout = () => {
    ...
    setUser(null);
    setRival(null); // Limpa o rival
    setShowBattle(false);
    setStatus("System Idle");
  };

  // --- AQUI ESTÁ A MÁGICA QUE FALTAVA ---
  const handleChallenge = (selectedRival: any) => {
    // Impede de lutar contra si mesmo
    if (selectedRival.id === user?.id) {
      alert("Protocolo inválido: Você não pode batalhar contra si mesmo.");
      return;
    }
    
    // Define o rival e abre a arena
  ...
  return (
    <div className="relative min-h-screen bg-[#050505] text-zinc-100 flex flex-col items-center justify-start py-12 px-6 font-mono overflow-y-auto">
      ...
      <motion.div animate={{ x: [0, 50, -50, 0], y: [0, -30, 30, 0], scale: [1, 1.1, 0.9, 1] }} transition={{ duration: 20, repeat: Infinity, ease: "easeInOut" }} className="fixed top-[-10%] left-[-10%] w-[500px] h-[500px] bg-purple-600/20 rounded-full blur-[120px] pointer-events-none mix-blend-screen opacity-70" />
      <motion.div animate={{ x: [0, -70, 30, 0], y: [0, 50, -50, 0], scale: [1, 1.2, 0.8, 1] }} transition={{ duration: 25, repeat: Infinity, ease: "easeInOut", delay: 2 }} className="fixed bottom-[-10%] right-[-10%] w-[600px] h-[600px] bg-emerald-500/20 rounded-full blur-[130px] pointer-events-none mix-blend-screen opacity-70" />

      {/* Interface Principal */}
      <motion.div initial={{ opacity: 0, scale: 0.95, filter: "blur(10px)" }} animate={{ opacity: 1, scale: 1, filter: "blur(0px)" }} className="w-full max-w-[420px] relative z-10 flex flex-col gap-8">
        
        {/* Header */}
        <div className="flex flex-col items-center space-y-4">
          <motion.div whileHover={{ scale: 1.05, rotate: 180 }} className="p-4 bg-zinc-900/50 rounded-2xl border border-zinc-800/50 shadow-[0_0_30px_-10px_rgba(16,185,129,0.3)] backdrop-blur-md">
            <Terminal className="w-6 h-6 text-emerald-400" />
          </motion.div>
          <div className="text-center">
            <h1 className="text-3xl font-bold tracking-tighter text-transparent bg-clip-text bg-gradient-to-br from-white via-zinc-200 to-zinc-600">DevSpirit</h1>
            <div className="flex items-center justify-center gap-2 mt-2">
              ...
              <p className="text-[10px] uppercase tracking-[0.2em] text-zinc-500 font-semibold">{status}</p>
            </div>
          </div>
        </div>
              ...
              {isLoading ? (
                ...
                <motion.div key="dashboard" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={handleLogout} className="p-2 text-zinc-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors"><LogOut size={20} /></button>
                  </div>

                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-zinc-900/50 border border-zinc-800/50 p-4 rounded-xl hover:border-emerald-500/30 transition-colors">
                    ...
                    <div className="bg-zinc-900/50 border border-zinc-800/50 p-4 rounded-xl hover:border-purple-500/30 transition-colors">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[10px] text-zinc-500 uppercase tracking-wider">XP Total</span>
                        <Activity className="w-3 h-3 text-purple-500 opacity-50" />
                      </div>
                  ...
                  <div className="mt-6 w-full">
                    <SpiritPet level={user.pet?.level || 1} xp={user.pet?.xp || 0} isMegaRare={user.pet?.isMegaRare} />
                  </div>
                  
                  {/* Botão de Batalha do Card Principal (Leva ao ranking) */}
                  <motion.div className="w-full py-4 mt-4 bg-gradient-to-r from-red-900/20 to-red-600/10 border border-red-500/20 rounded-xl flex items-center justify-center gap-2">
                    <Swords className="w-5 h-5 text-red-400/50" />
                    <span className="text-red-200/50 font-bold tracking-widest text-xs">PVP ARENA ONLINE</span>
                  </motion.div>
                </motion.div>
              ) : (
        ...
        {/* LEADERBOARD COM O CLICK ATIVADO */}
        {user && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
            {/* AQUI ESTAVA FALTANDO: onChallenge={handleChallenge} */}
            <Leaderboard onChallenge={handleChallenge} />
          </motion.div>
        )}
      ...
      {/* MODAL DE BATALHA */}
      <AnimatePresence>
        {showBattle && user && rival && (
          <BattleArena myUser={user} rivalUser={rival} onClose={() => setShowBattle(false)} />
        )}
      </AnimatePresence>
